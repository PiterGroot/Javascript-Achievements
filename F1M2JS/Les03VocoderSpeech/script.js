    ...
    function makeThisSentence(){
        let rand = randomizer(arrayLength);
        let rand1 = randomizer(arrayLength);
        let rand2 = randomizer(arrayLength);
        let woord1 = onderwerp[rand];
        let woord2 = werkwoord[rand1];
        let woord3 = restwoord[rand2];
    ...
    function sayItLoud (textString) {
      ...
      let randVoice = randomizer(arrayLength);
      message.voice = voices[randVoice];
      let randPitch = randomizer(arrayLength);
      message.pitch = randPitch; // range between 0 and 2
      let randRate = randomizer(arrayLength);
      message.rate = randRate; // range between 0.1 (lowest) and 10 (highest) 
      window.speechSynthesis.speak(message);
    }
    
    
    
    const onderwerp = ["He", "James", "Sam"];
    const werkwoord = ["cycles", "eats", "sleeps"];
    const restwoord = ["fast", "slow", "bad"];
    ...
    let reset = "https://i.pinimg.com/originals/16/9a/8d/169a8dfcb402415d343481e7143f932c.jpg"
